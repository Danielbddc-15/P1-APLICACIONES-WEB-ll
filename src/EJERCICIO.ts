...
function buscarGastoPorID(ID: number, callback: (err: Error | undefined, res: any | undefined) => void) {
    const gastoEncontrado = gastos.find(gasto => gasto.IDGASTOS === ID);
    callback(undefined, gastoEncontrado);
}

const ID: number = 3; // Cambié el tipo de ID a number para que coincida con el tipo de ID en los datos de gastos
if (ID > 5) {
